...
create_doks_s3() {
  ...
  create_secret "$secret_name" "$access_key" "$secret_key"
  retcode=$?
  if [ "$retcode" -ne 0 ]; then
    echo "Exiting..."
    exit 1
...
create_readymade_minio() {
  ...
  if [[ "$use_existing_minio" != "Y" ]] && [[ "$use_existing_minio" != "y" ]] || [[ "$rel_name" == "" ]]; then
    ...
    if [[ -z ${input_config} ]]; then
      read -r -p "you any available minio server (y/n) ('Y') : " default_minio
    fi
  ...
  cmd="kubectl get pod $mc_pod_nm -o jsonpath='{.status.phase}' 2>/dev/null | grep -e Running"
  wait_install 10 "$cmd"
  if ! kubectl get pod $mc_pod_nm -o jsonpath="{.status.phase}" | grep -q Running; then
    echo "Minio client container taking more time than usual to be in Ready state, Exiting.."
    return 1
  fi
  ret_val=$(kubectl exec -it $mc_pod_nm -- mc alias set minio $url $access_key $secret_key 2>> >(logit))
  ret_msg=$(kubectl exec -it $mc_pod_nm -- mc mb minio/$bucket_name)
  #create bucket
  #ret_val=$(s3cmd --config s3cfg_config mb s3://"$bucket_name" 2>&1)
  ret_code=$?
  if [[ "$ret_code" -ne 0 ]]; then
  ...
  kubectl delete pod $mc_pod_nm 1>> >(logit) 2>> >(logit)
